...
for link in BeautifulSoup(urlgrab, parseOnlyThese=SoupStrainer('a')):
    if link.has_key('href'):
            if 'transgenic' in link['href']:
                 ...
                 for link in BeautifulSoup(urlgrab2, parseOnlyThese=SoupStrainer('a')):
                     if link.has_key('href'):
                     #if 'lbl.gov' in link['href']:
                         if 'imagedb3' in link['href']:
                             elementNumber = link['href']
                             elementNumber = re.sub('.*experiment_id=','',elementNumber)
                             elementNumber = re.sub('.organism_id','',elementNumber)
                 ...
                 for el in soup2.findAll({'p' : True}):
                     for item in el:
                         if re.search('ouse genom', str(item)):
                             ...
                             output += "{chr:\""+chrom+"\", start: "+start+", end: "+end+", url: \""+url2+"\", genes: "+genesAsJsonArray+", elementid: "+elementNumber+" },"
